...
class System extends Component {
  ...
  sumTotalPowerUsage() {
    ...
    this.state.subSystems.map(subSystem => totalPowerUsage += subSystem.power.numberUsed);
  ...
  updatePowerUsage(event, id) {
    ...
    currentSubSystem.power.percentUsed = currentPercentage;
    currentSubSystem.power.numberUsed = Math.floor(this.state.totalPowerUnits * currentPercentage);
  ...
  render() {
    ...
    return(
      <SystemContainer Danger={isDanger === true ? `0` : `240`}>
        <SystemTitle>{this.state.systemName}</SystemTitle>
        <SystemInformation>
          <h2>System Stats</h2>
          <ul>
          ...
          <SubSystemList>
            {this.state.subSystems.map((subSystem, index) => {
              return(
                <SubSystem
                  key={subSystem.id}
                  systemName={subSystem.name}
                  powerUsage={subSystem.power.percentUsed}
                  powerUsed={subSystem.power.numberUsed}
